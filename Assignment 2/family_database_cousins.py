...
User can create a rabbit, choose their age, list the rabbit out and create a relationship with the parents and cousins and list the family out.
"""
...
def cousins(rabbit, parent, kitten):
    """
    Create cousins relationship that have the same parents.
    User need to enter a name that exist in the database.
    """
    ...
    while True:
        rabbit_name = input("Input the rabbit's name:\n")
        if rabbit_name in rabbit:
            # Initialize an empty list to store the cousins
            cousin_list = [] 
            # Check if the rabbit has parents
            if rabbit_name in kitten:
                parents = kitten[rabbit_name]
            else:
                parents = []

            for parent_name in parents:
                # Check if the parent has grandparents
                ...
                for grandparent_name in grandparents:
                    # Check if the grandparents have siblings
                    if grandparent_name in parent:
                        siblings = parent[grandparent_name]
                    else:
                        siblings = []

                    for sibling in siblings:
                        # Ensure the sibling isn't the rabbit's parent
                        if sibling != parent_name:
                            if sibling in parent:
                                cousins = parent[sibling]
                            else:
                                cousins = []

                            for cousin in cousins:
                                # Ensure the cousin isn't the rabbit itself
                                if cousin != rabbit_name:
                                    # Add the cousin to the list
                                    cousin_list.append(cousin)

            print(f"Cousins of {rabbit_name}:")
            if cousin_list:
                # Sort the counsin list
                for cousin in sorted(cousin_list):
                    print(cousin)
            break
        else:
            print("That name is not in the database.")
...
def main():
    ...
    User can choose to create a rabbit, choose their age, list the rabbit out and create a relationship with the parents and cousins or list the family out.
    """
    ...
    parent={}
    while True:
        # It will repeat until the user input a number from 0 to 6
        print("==================================")
