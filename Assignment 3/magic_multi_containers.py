...
The program now includes magic multi containers which holds multiple compartments and calculates its total empty weight based on the compartments.
"""
...
class Magic_Multi_Container(Multi_Containers, Magic_Containers):
    """
    A class representing a Magic_Multi_Container    
    """
    def __init__(self, name, compartments):
        """
        Initialize the Magic_Multi_Container with a name and its compartments.
        Arguments -> name (str): The name of the magic multi container
        compartments(list): A list of compartment objects (containers) inside the magic multi-container.
        """
        # Use Multi_container to initialize the compartments and empty weight
        Multi_Containers.__init__(self, name, [])
        self.compartments = compartments
        # loop through each compartment and add its empty weight to the total empty weight of the Magic_Multi_Container
        for compartment in self.compartments:
            self.empty_weight += compartment.empty_weight

    def add_item(self, item) -> bool:
        """
        Iterates through each compartment to add the item.
        Arguments -> item: The item to be added to one of the compartments.
        Returns->bool: True if the item was successfully added to one of the compartments, otherwise False.
        """
        # Iterate through each compartment to check if the item can be added to any of the compartments
        for compartment in self.compartments:
            if compartment.add_item(item):
                return True
        return False
    ...
    def __str__(self):
        """
        Return a string representation of the Magic_Multi_Container   
        """
        # Return the name, total weight, empty_weight and capacity
        return f"{self.name} (total weight: {self.empty_weight}, empty weight: {self.empty_weight}, capacity: {self.current_weight}/{self.max_weight_capacity})"

def loot_items(selected_container):
    """
    Allow User to input the name of an item and attempt to add it to the selected container.
    """
...
def load_magic_multi_containers(file, containers):
    """
    Load magic multi containers from a CSV file and return a list of Magic_Multi_Containers objects.
    Arguements -> file (str): The CSV file containing Magic_Multi_Containers data.
    containers (list): A list of Container objects. The compartments of each Magic_Multi_Container are selected from this list based on their names.
    Returns:A list of Magic_Multi_Container objects. Each Magic_Multi_Container is initialised with its name and compartments.
    """
    magic_multi_containers = []
    with open(file, newline='') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)
        # Loop through each row in the CSV file, get the name of the Magic_Multi_Container
        for row in reader:
            name = row[0].strip()  
            compartments = []
            # Loop through the compartment names listed in the row start from row 1 and remove extra spaces from th compartment name
            for compartment_name in row[1:]:
                compartment_name = compartment_name.strip()
                # Find the container with the matching name in the list of containers
                for container in containers:
                    if container.name == compartment_name:
                        # Add this container's compartments to the list
                        for container in container.compartments :
                            compartments.append(container)
            magic_multi_containers.append(Magic_Multi_Container(name, compartments))
...
def main():
    ...
    magic_multi_containers = load_magic_multi_containers('magic_multi_containers.csv', multi_containers)

    all_containers = containers + multi_containers + magic_containers + magic_multi_containers
    selected_container = None

    print(f"Initialised {len(items + containers + multi_containers + magic_containers + magic_multi_containers)} items including {len(containers + multi_containers + magic_containers + magic_multi_containers)} containers.\n")
