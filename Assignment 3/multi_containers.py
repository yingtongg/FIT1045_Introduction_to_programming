...
class Multi_Containers(Containers):
    ...
    def __init__(self, name, compartments):
        ...
        Compartments (list of Containers): A list of individual container compartments within the multi-container.
        """
    ...
    def list_items(self):
        """
        The method loops through compartments in a multi-container and create a formatted string that lists each compartment and its items.
...
def main():
    ...
    multi_compartments = load_multi_containers('multi_containers.csv', containers)

    all_containers = containers + multi_compartments
    selected_container = None

    print(f"Initialised {len(items + containers + multi_compartments)} items including {len(containers + multi_compartments)} containers.\n")
